...
contract SimpleBank {
    ...
    // Modifiers 
    modifier isEnrolled { // checks if user is enrolled
      require(enrolled[msg.sender], "Please enroll first"); 
      _;
    }

    modifier isNotEnrolled {
      require(!enrolled[msg.sender], "Your already enrolled");
      _;
    }
    ...
    function getBalance() public view isEnrolled returns (uint _balance) {
    ...
    function enroll() public isNotEnrolled returns (bool ){
      ...
      emit LogEnrolled(msg.sender); // emits event of enrolled student
